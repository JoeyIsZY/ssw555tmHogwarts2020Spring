...
def us05_marriage_before_death(repo):
    ...
    for fam in repo.families.values():
        ...
        for indi in repo.individuals.values():
            if indi.indi_id == hus_id:
                hus_ddate = indi['DEAT']['detail']
                if hus_ddate != 'NA':
                    if marriage_date > hus_ddate:
                        error_message = f'Husband:<{indi.indi_id}> dead before he married. '
                        error.append(('ERROR', 'INDIVIDUAL', 'US05', (indi['DEAT']['line'], fam['MARR']['line']), indi.indi_id, error_message))

            if indi.indi_id == wife_id:
                wife_ddate = indi['DEAT']['detail']
                if wife_ddate != 'NA':
