            ...
            for chil in fam.repo['CHIL']['detail']:
                ...
                if chil_bir['detail'] != 'NA':
                    ...
                    if not wife.alive and chil_bir['detail'] > wife.repo['DEAT']['detail']:
                        err.append(('ERROR', 'FAMILY', 'US09', (wife.repo['DEAT']['line'], chil_bir['line']), fam.fam_id, f"Mother's death date {wife.repo['DEAT']['detail']:%Y-%m-%d} occurs before child {chil} birth {chil_bir['detail']:%Y-%m-%d}."))
                    if not husb.alive and (chil_bir['detail'] - husb.repo['DEAT']['detail']).days > 270:
                        err.append(('ANOMALY', 'FAMILY', 'US09', (husb.repo['DEAT']['line'], chil_bir['line']), fam.fam_id, f"Father's death date {husb.repo['DEAT']['detail']:%Y-%m-%d} occurs more than 9 month before child {chil} birth {chil_bir['detail']:%Y-%m-%d}."))
    return err
